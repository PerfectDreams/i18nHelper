...
class I18nHelperPlugin : Plugin<Project> {
        ...
        with(target) {
            ...
            tasks.register(TASK_NAME) {
                ...
                doLast {
                    ...
                    val map = loadTranslations(File(this.project.projectDir, extension.languageSourceFolder.get()))
    ...
    fun loadTranslations(file: File): Map<String, Any> {
        val map = mutableMapOf<String, Any>()
        loadTranslationsFromFolder(file, map, true)
        return map
    }

    fun loadTranslationsFromFolder(file: File, map: MutableMap<String, Any>, skipLanguageYaml: Boolean) {
        val files = file.listFiles() ?: error("Not a valid file!")

        for (childFile in files) {
            if (childFile.isDirectory) {
                loadTranslationsFromFolder(childFile, map, false)
                ...
                loadTranslationFileToMap(childFile, map)
            }
        }
    }

    fun loadTranslationFileToMap(file: File, map: MutableMap<String, Any>) {
